...
func interact():
	match inter:
		1:
			...
			SignalManager.show_choice2.emit("I wonder what is inside.", "Look", "Don't", Open)
		2:
			SignalManager.show_text.emit("Did the drawer cut it off? Does it belong to someone trapped here?..")
			inter +=1
		3:
		...
		4:
			inter +=1
			SignalManager.show_text.emit("Ok, one more peek... ")
...
func Open():
	...
	SignalManager.show_text.emit("(I need to leave... as soon as I find Margaret... Alice... and Dan...)")
	animated_sprite_2d.play("close")
